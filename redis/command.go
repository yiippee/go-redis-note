...
type baseCmd struct {
	_args []interface{} // 命令的参数数组，比如ping，(set "123") 等
	err   error // 读取命令回复时的错误信息

	_readTimeout *time.Duration // 读回复的超时时间
}
...
// 参见https://redis.io/commands/command
type CommandInfo struct {
	Name        string // 命令名称
	Arity       int8   // 参数数量，正数代表整个命令的参数个数，包括命令本身；负数代表参数的最小个数，因为可能有多个key的情况
	Flags       []string // 描述命令的状态，只读，可写，pubsub 等等
	FirstKeyPos int8 // 第一个key的位置。Position 0 is always the command name itself.
	LastKeyPos  int8 // 最后一个key的位置
	StepCount   int8 // Key step count allows us to find key positions in commands like MSET where the format is MSET _key1_ _val1_ [key2] [val2] [key3] [val3]
	ReadOnly    bool
}
